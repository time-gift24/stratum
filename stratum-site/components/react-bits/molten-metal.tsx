...
/*
 * MoltenMetal — React Bits 底稿（reactbits.dev，MIT），TypeScript 化。
 * 本项目适配点（相对原版）：
 * 1. 作为背景场使用：容器 pointer-events-none，指针轨迹监听挂 window；
 * 2. TS strict 类型化；其余 shader 与逻辑保持原样。
 */
...
export default function MoltenMetal({
  ...
  useEffect(() => {
    ...
    // 背景场：指针轨迹挂 window，而非 canvas（canvas 被内容层覆盖）
  ...
  return (
    <div
      ref={containerRef}
      aria-hidden="true"
      className={cn_container(className)}
    />
  );
}

function cn_container(className: string): string {
  return `pointer-events-none absolute inset-0 overflow-hidden ${className}`.trim();
}
