...
- Proof and content: only API-backed names, provider/model identity, tool counts, timestamps, schema, revisions, blockers, violations, and one-shot connection-test results. StatusChip renders only real API fields (credential_configured, is_default); Agent definitions have no status field, so Agent cards carry no badge by design.
