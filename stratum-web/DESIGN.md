...
- 本体画布编辑器（`/ontologies/[id]`）满铺无头部栏：控制全部收进悬浮 pill（`ontology-chrome.tsx`）——左上返回+标题+保存状态，右上选中工具条+全局动作（视图切换/新增/保存）；文字只留标题与真实状态，其余一律图标+tooltip，需要输入的编辑（改名/描述/cardinality/聚焦深度）收进图标 Popover，不用右侧常驻面板。草稿/保存错误/违例/聚焦提示是顶部居中浮层，只在有事时出现。
