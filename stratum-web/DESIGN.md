...
- 唯一固定外壳是顶部 SiteNav：`components/chrome/site-chrome.tsx` + react-bits `SiteNav`（数据驱动），入口为对话 / 仪表盘（`/studio`）/ 本体 / Excalidraw，右端是 actions 槽的图标操作（主题切换 + 设置入口 `/studio/settings/providers`，纯图标无文字）。受保护的 `components/react-bits/*` 实现改动需用户批准（本次 actions 槽已获批准）。
