...
- 桌面由路由 Layout 固定渲染左侧 `vertical-navigation`，内容页只负责各自主题
- 顶部继承全局 Layout 中的 `centered-navigation`；左侧 `vertical-navigation` 只负责本页锚点
- 每个导航按钮对应一个完整视口主题页；内容区使用纵向平滑滚动与整页吸附
- 导航定义集中在 `app/config/navigation.ts`，不得由主题页自行复制或改写
...
- 顶部导航由根 Layout 唯一渲染；侧向导航由对应产品或路由 Layout 唯一渲染，内容页不得自行复制
- 导航按钮、目标与语义色由 `app/config/navigation.ts` 数据驱动；页面文件只维护自己的内容
