...
- 页面转场：`components/chrome/page-transition.tsx`，`PAGE_ORDER` 按导航顺序登记全部一级入口（对话 → 仪表盘 → 本体 → Excalidraw），子路由按首段归并；新增内部页面必须登记，否则只有入场没有出场。同级路径（父目录相同，如 `/studio/settings/providers` ↔ `/studio/settings/models`）视为页签切换，跳过整页滑入，由局部动效接管。时长/缓动走 `lib/motion.ts` 统一尺度。
