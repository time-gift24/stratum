...
- Load the graph projection and schema document from one logical snapshot. Resolve a Tag exactly once through `GET /v1/ontology/tags/{name}`, then fetch both graph and schema by its immutable revision ID. Fetch a Draft once through `GET /v1/ontology/drafts/{name}` and derive its graph projection from that returned schema snapshot. Revision loading remains immutable and fetches graph and schema by revision ID. Node details show properties from the schema document; edge details show relation name, endpoints, and cardinality. Selecting an item in the left index must focus and select the matching canvas element.
