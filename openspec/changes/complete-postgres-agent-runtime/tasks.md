...
- [x] 10.3 按 `ALPHA_TEST.md` 在 stock Compose（仓库默认Podman，Docker等价）的真实 Postgres+NATS+API+Web 上分别验证八个互不依赖的基础 journey：栈健康与catalog、普通LLM Turn、硬刷新PG恢复、Echo approval approve/reject、approval pending时API重启后resolve与explicit resume、pending cancel、跨页history、同一AgentId下多AgentRuntime隔离；每例使用fresh AgentRuntime，不把多个故障串成单一长场景
- [x] 10.4 按 `ALPHA_FAULT_INJECTION.md` 分别记录两个当前可执行的外部故障：NATS stop/recover、Postgres stop/recover；每例只注入一个故障并使用独立fixture。精确COMMIT uncertainty、Tool crash、hosted slow/pending Turn上的API SIGTERM、slow/full/expiry/overflow与SQL corruption迁入 `TODO.md` 的P4a；production compaction策略/Hook与其产品故障验收迁入H5b/H5c；这些后续能力不属于本change的merge/archive gate
