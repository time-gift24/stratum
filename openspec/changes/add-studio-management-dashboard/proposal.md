...
Stratum 已收敛为 Postgres-first 执行 runtime 与只读 template catalog，但缺少面向开发者和管理员的管理入口，也没有可写的资源管理 API；调整定义仍需直接修改文件并重启服务。此前的文件 catalog / 旧 HostState 实现已无法与当前 runtime 共存，需要将 Studio 配置移到与执行账本隔离的 PostgreSQL 持久化模块。
...
- 新增独立 `stratum-studio` library crate 与 Studio database；其具体 `StudioStore` 是 Provider、Model 和 Agent definition 的唯一管理持久化接口，不修改 `stratum-postgres` 的执行账本。
...
- `agent-definition-management`: Agent 定义的持久化 CRUD、版本不变量、校验、引用冲突与管理 API 契约。
- `llm-resource-management`: Provider 与 Model 的持久化 CRUD、secret 安全、连接测试、schema 驱动参数与运行时 catalog 刷新。
...
- **后端**：`stratum-api` 新增 `/v1/agent-definitions`、`/v1/providers` 与其 model/test 子资源的 REST API、OpenAPI DTO 与错误映射；应用装配支持 Studio database 的持久化变更生效。
- **领域与存储**：新增 `stratum-studio`，以独立 database/migration history 持久化管理 catalog；`stratum-postgres` 继续只承载执行真相。Provider secret 必须以 `secrecy::Secret` 处理且永不通过读取 API 回显。
