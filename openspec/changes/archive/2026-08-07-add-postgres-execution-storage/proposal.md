> **Superseded by `complete-postgres-agent-runtime`.** 本 change 的双后端/三表方案已被取代：Postgres 成为唯一执行真相（四表：`agents`、`agent_state`、`durable_events`、`transcript_compactions`），`stratum-store` 与 filesystem 执行后端整体删除。本文件仅作历史记录归档，其 delta specs 不再同步到 main specs。
