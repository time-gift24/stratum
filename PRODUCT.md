...
当前产品提供对话界面、Excalidraw 白板和本体管理（列表 + 画布编辑器）三个界面。成功意味着使用者可以清楚地发起任务、理解必要反馈，并让 Agent 可靠地继续推进，而不是只获得一次性回答。本体管理前端按 `docs/ontology/API.md` 契约实现，后端联调待落地，不计入已交付的端到端能力。
...
- 产品当前界面包括对话页面（含新建对话入口）、Excalidraw 白板，以及面向编排侧的本体列表与画布编辑器；本体能力前端按 API 契约实现，后端联调待落地，不视为已端到端交付。
...
4. **一个产品，界面各守边界。** 对话面向任务执行者，画布面向配置与编排。
