...
本文记录 `stratum-api` 首版 Agent 对话 API 的历史设计。当前 Session、事件与恢复协议以
[`PROTOCOL.md`](PROTOCOL.md) 为准；本文中的 API 细节已按该基线更新。
...
- Session 状态存储、Agent 列表、删除、配置修改或模板管理 API。
...
- 前端对两个来源进行投影，并按 `(agent_id, message_seq)` 去重稳定消息。
...
| `POST /v1/agents/{agent_id}/messages` | 发送用户文本并启动 Turn | `202 TurnAccepted` |
| `GET /v1/agents/{agent_id}/events` | retained + live Agent SSE | `200 text/event-stream` |
| `POST /v1/agents/{agent_id}/resume` | 恢复未完成 Turn | `202 TurnAccepted` |
...
6. 返回 `201`、`agent_id`、`session_id` 和 `turn_id`。

首条消息持久化失败时清理本次新建目录，不加入 registry，也不向调用方暴露
`agent_id`。消息已经持久化后，即使 provider 随后失败，也保留这个有效的非空对话，
失败通过 `AgentEvent::Failed` 发布。

### durable Turn acceptance

现有 `Agent::run_turn()` 调整为：

1. 接收 host 提供的 Session context，建立 active guard、Turn ID 与取消通道。
2. 在返回 `TurnId` 前同步发布 `AgentEvent::Started` 和首条
   `AgentEvent::Message`。
...
4. 按 Agent 消息内的 `message_seq` 建立历史，并与暂存事件合并。
5. 对稳定消息按 `(agent_id, message_seq)` 去重。
...
SSE 先于历史分页建立，因此分页期间发生的事件不会落入窗口。`message_seq` 只排序所属 Agent 的
...
- 请求 span 只记录 route、method、status、agent_id、agent_name、session_id、turn_id、cursor 和耗时。
- 错误只在最终处理边界记录一次。
