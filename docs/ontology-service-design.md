...
- 通过 `stratum-filesystem` 将可编辑 schema draft 保存为 JSON 文件。
...
Migration 存放在 `stratum-ontology-mysql/migrations/`，仅由 SQLx CLI 手动管理。应用启动时不调用 `sqlx::migrate!`，不建表，也不修改数据库 schema。
...
- `stratum-ontology`：领域 newtype 与类型、规范 schema codec、draft 访问协议、校验与 Service 用例；不依赖 Axum 或 SQLx。
- `stratum-ontology-mysql`：SQLx/MySQL 持久化实现与 migration。
- `stratum-ontology-api`：Axum `router()`、HTTP DTO 与错误映射。未来的应用宿主负责提供 MySQL pool 和虚拟文件系统。

现有 `stratum-filesystem::Filesystem` 是 draft 文件边界。Ontology Service 将逻辑 draft 名称映射到固定虚拟 draft 目录，永不接受主机路径。
...
- MySQL 8 集成测试放在 `stratum-ontology-mysql/tests/`，默认 workspace 测试中标记为忽略，并使用该 crate 的 `docker-compose.test.yml`。测试环境先运行 SQLx CLI migration。
